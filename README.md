...
**حماية `.htaccess`:** كل مجلد `data/` (فيه ملفات JSON للتقدم) و`sandbox/` (فيه ملفات التنفيذ المؤقتة) في كل مسار محمي بـ `.htaccess` بيرفض أي طلب مباشر ليه من المتصفح (`Require all denied`) — الكود بنفسه بيقرا/يكتب الملفات دي من غير مشكلة، لكن محدش يقدر يطلبها مباشرة بالرابط. اتأكد إن استضافتك بتفعّل `AllowOverride All` (افتراضي على أغلب الاستضافات، ومنها Hostinger) عشان `.htaccess` يشتغل فعليًا — تقدر تتأكد بفتح `deploy-check.php` أو بمحاولة فتح رابط ملف JSON مباشرة وتتأكد إنه بيرجّع 403.
...
Vercel، Netlify، وأي منصة Serverless مبنية لـ Next.js/Node/Static من الأساس — **مفيهاش PHP runtime خالص**، وحتى لو كان فيه، فكرة `proc_open` بتشغّل ملف تنفيذي (php-cli/python) دي بالظبط اللي منصات الـ Serverless بتمنعها أمنيًا. المشروع ده محتاج سيرفر تقليدي (Apache/Nginx + PHP-FPM أو mod_php) شغال باستمرار. **ده مش قابل للحل بأي إعداد أو تجهيز — لو خطتك Vercel، غيّرها لهوستنج تقليدي أو VPS.**

### 🩺 أول حاجة تعملها بعد أي رفع: `deploy-check.php`

المشروع فيه صفحة تشخيص حقيقية على `yourdomain.com/deploy-check.php` — بتشغّل فحوصات فعلية (مش بس تقرأ إعدادات) وتقولك فورًا: نسخة PHP، هل `proc_open` شغّال فعليًا (بتجرّب تشغّل عملية حقيقية)، هل `pdo_sqlite` متاح (للـ Database Playground)، هل مجلدات `data/` و`sandbox/` قابلة للكتابة فعليًا في كل مسار (23 مجلد)، والـ Sessions شغالة. **افتحها فور ما ترفع المشروع على أي استضافة، قبل ما تعتمد إن حاجة شغالة.** امسحها أو احميها بباسورد بعد التأكد، لإنها بتكشف تفاصيل إعداد السيرفر.

### ⚠️ استضافة مشتركة (Shared Hosting) — محتاج تتأكد الأول

معظم الاستضافات المشتركة الرخيصة (Hostinger, Namecheap, Bluehost العادية...) **بتقفل `proc_open`/`exec` افتراضيًا** على كل حساباتها كإجراء أمني عام — مش حاجة تقدر تفعّلها بنفسك من غير دعم فني، لأنها مضبوطة في `php.ini` بتاع السيرفر كله. لو اشتريت استضافة وطلع `proc_open` مقفول، المنصة **مش هتنهار** — كل الـ Playgrounds هترجّع رسالة واضحة "الاجرا الحقيقي متوقف على السيرفر ده" بدل ما تفشل بصمت أو ترجّع "(no output)" محيّرة، لكن الميزة الأساسية (تشغيل كود حقيقي) مش هتشتغل خالص.
...
3. بعد ما ترفع، افتح `deploy-check.php` فورًا — هيقولك بالظبط أنهي حاجة شغالة وأنهي لأ، من غير ما تدوّر درس بدرس.

استضافات زي **A2 Hosting** أو **InterServer** بتسمح غالبًا بـ SSH وexec على الباقات الأعلى، لكن ده بيختلف حسب الخطة والوقت. لو معاك هوستنج (Hostinger) بالفعل، ارفع المشروع (عن طريق Git في hPanel أو File Manager)، اضبط الـ Document Root على مجلد المشروع، اختار نسخة PHP 8.0 أو أحدث من إعدادات الاستضافة، وافتح `deploy-check.php` فورًا.
